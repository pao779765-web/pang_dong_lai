...
>
> 生成时间：2026-08-02T08:43:31.726Z

## 1. 结论

当前检索通过 39/54 题，综合通过率为 **72.2%**。Query 改写启用前控制组为 39/54，本次变化 0 题；新增通过：无；退步：无。本次只运行确定性的检索与路由，不调用 DeepSeek，因此不会产生费用；“生成越界”只记录检索结果可能带来的风险，不把风险写成已经发生的回答错误。
